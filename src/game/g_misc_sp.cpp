...
// Decomp: CoDSP_rdBlackOps.map.c (misc_EntInfo)
void __cdecl misc_EntInfo(gentity_s *self, float *source)
{
    ...
    if ( !self && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_misc_sp.cpp", 61, 0, "%s", "self") )
        __debugbreak();
    EntinfoPosAndScale(self, source, xyz, &infoScale, &dist);
    if ( !self->classname
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_misc_sp.cpp", 65, 0, "%s", "self->classname") )
    {
        __debugbreak();
    }
    ...
    if ( g_entinfo->current.integer == 4 || g_entinfo->current.integer == 5 )
    {
        ...
        G_AddDebugString(xyz, color, infoScale * 0.75, fmtMsg, 0);
    }
    else
    {
        if ( self->targetname )
            slStr = SL_ConvertToString(self->targetname, SCRIPTINSTANCE_SERVER);
        else
            slStr = "<noname>";
        fmtMsg2 = va("%i : %s : %i : %s", self->s.number, slStr, self->health, classname);
        G_AddDebugString(xyz, color, infoScale * 0.75, fmtMsg2, 0);
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (EntinfoPosAndScale)
void __cdecl EntinfoPosAndScale(gentity_s *self, float *source, float *pos, float *textScale, float *dist)
{
    float delta[3];

    if ( !self && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_misc_sp.cpp", 26, 0, "%s", "self") )
        __debugbreak();
...
// Decomp: CoDSP_rdBlackOps.map.c (EntInfo_Vehicle)
...
// Decomp: CoDSP_rdBlackOps.map.c (EntInfo_Item)
void __cdecl EntInfo_Item(gentity_s *self, float *source)
{
    ...
    if (!self && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_misc_sp.cpp", 108, 0, "%s", "self"))
        __debugbreak();
    ...
    if (dist <= MY_MAX_DIST)
    {
        ...
        for (idx = 0; idx < 2; ++idx)
        {
            if (self->item[idx].index)
            {
                ...
                weaponName = BG_WeaponName(self->item[idx].index);
                fmtMsg = va("%i: %s (%i + %i)", self->s.number, weaponName, scale, ammoCount);
                G_AddDebugString(pos, color, textScale * 0.80000001, fmtMsg, 0);
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetEntInfoScale)
float __cdecl G_GetEntInfoScale()
{
    return CG_GetViewZoomScale() * g_entinfo_scale->current.value;
}

// Decomp: CoDSP_rdBlackOps.map.c (SP_info_notnull)
void __cdecl SP_info_notnull(gentity_s *self, SpawnVar *v)
{
    G_SetOrigin(self, self->r.currentOrigin);
}

// Decomp: CoDSP_rdBlackOps.map.c (SP_sound_blend) — dynamic spawn for rain/ambient blend ents.
void __cdecl SP_sound_blend(gentity_s *self, SpawnVar *v)
{
    G_SetOrigin(self, self->r.currentOrigin);
...
// Decomp: CoDSP_rdBlackOps.map.c (SP_light)
void __cdecl SP_light(gentity_s *self, SpawnVar *spawnVar)
{
    ...
    if ( !spawnVar->spawnVarsValid
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_misc_sp.cpp",
                    169,
                    0,
                    "%s",
...
// Decomp: CoDSP_rdBlackOps.map.c (TeleportPlayer)
void __cdecl TeleportPlayer(gentity_s *player, float *origin, float *angles)
{
    ...
    if ( !player->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_misc_sp.cpp", 229, 0, "%s", "player->client") )
    {
        __debugbreak();
    }
    ...
    *tmp4 = *origin;
    ...
    tmp3 = player->client->ps.origin;
    player->r.currentOrigin[0] = *tmp3;
    player->r.currentOrigin[1] = tmp3[1];
    player->r.currentOrigin[2] = tmp3[2];
    if ( linked )
        SV_LinkEntity(player);
}

// Decomp: CoDSP_rdBlackOps.map.c (ScrNotify_FaceEvent)
...
// SP: spawn system debug is MP-only; no-op for script compatibility.
