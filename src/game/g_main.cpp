...
#include <server_sp/sv_snapshot_sp.h>
...
// static_assert(sizeof(level_locals_t) == 0x5620, "SP level_locals_t must match CoDSP (0x5620)");

// CoDSP: entity snapshot debug counters live in g_main.obj (not sv_msg_write_mp.obj).
...
int num_zero_gravity_volumes;
char zero_gravity_names[MAX_ZERO_GRAVITY_VOLUMES][32];
bool zero_gravity_on[MAX_ZERO_GRAVITY_VOLUMES];
...
void G_FreeAnimTreeInstances()
{
    ...
    for ( i = 0; i < 16; ++i )
    {
        if ( g_scr_data.actorXAnimTrees[i] )
        {
            XAnimFreeTree(g_scr_data.actorXAnimTrees[i], 0, SCRIPTINSTANCE_SERVER);
            g_scr_data.actorXAnimTrees[i] = 0;
        }
    }
    if ( g_scr_data.actorBackupXAnimTree )
    {
        XAnimFreeTree(g_scr_data.actorBackupXAnimTree, 0, SCRIPTINSTANCE_SERVER);
        g_scr_data.actorBackupXAnimTree = 0;
    }
#endif
}

// Decomp: CoDSP_rdBlackOps.map.c:6073391
void __cdecl    G_InitGame(int levelTime, int randomSeed, int restart, int registerDvars)
{
    ...
    if (!restart)
    {
...

        {
            ...
            GScr_LoadScriptsAndAnims(SCRIPTINSTANCE_SERVER);
        }
    }
    ...
    num_zero_gravity_volumes = 0;
    memset(zero_gravity_names, 0, sizeof(zero_gravity_names));
    memset(zero_gravity_on, 0, sizeof(zero_gravity_on));
    ...
    G_InitPathnodeScriptLinks(restart);
    ...
    Com_Printf(15, "-----------------------------------\n");
    G_InitTargets();
    level.loading = LOADING_LEVEL;
    level.bScrInitOnLoad = 1;
...
void G_RegisterDvars()
{
...
#ifdef KISAK_SP
    // Decomp: BlackOps.singleplayer.c — SV_Init owns g_gametype; G_RegisterDvars must not Reregister as MP tdm.
    if ( !g_gametype )
        g_gametype = _Dvar_RegisterString("g_gametype", "cmp", 0x24u, "The current campaign");
    _Dvar_RegisterBool("sv_saveOnStartMap", 1, 4100, "Save at the start of a level");
    _Dvar_RegisterString("ui_campaign", "american", 0x1000u, "The current campaign");
    // Decomp: BlackOps.singleplayer.c G_RegisterDvars @ 0x007E0C00
    _Dvar_RegisterInt("g_player_maxhealth", 100, 10, 2000, 0x2000u, "Maximum player health");
    _Dvar_RegisterFloat(
    ...
    _Dvar_RegisterFloat(
        ...
        1000.0f,
        0x3080u,
        "Player radius damage multiplier");
    _Dvar_RegisterFloat(
    ...
    _Dvar_RegisterBool("player_deathInvulnerableToMelee", 0, 0x2080u, "Death invulnerability applies to melee");
    _Dvar_RegisterBool("player_deathInvulnerableToProjectile", 1, 0x2080u, "Death invulnerability applies to projectiles");
#else
    ...
    g_speed = _Dvar_RegisterInt("g_speed", 190, 0x80000000, 0x7FFFFFFF,
#ifdef KISAK_SP
        0x3000u,
#else
        0,
    ...
    g_debugaccuracy = _Dvar_RegisterInt("g_debugaccuracy", 0, 0, 1, 0x80u, "");
    ...
    ai_accuracyDistScale = _Dvar_RegisterFloat(
    ...
    ai_debugPlayerLOS = _Dvar_RegisterBool("ai_debugPlayerLOS", 0, 0x80u, "");
    ...
    g_reloading = _Dvar_RegisterInt("g_reloading", 0, 0, 10, 0, "Save-game reload state");
...
// Decomp: CoDMPServer.c:395526 / CoDSP_rdBlackOps.map.c (825C94D0)
...
#ifdef KISAK_SP
    anims = Scr_HACKGetGameGenericHumanTree().anims;
    iassert(anims);
    for ( int i = 0; i < 16; ++i )
        g_scr_data.actorXAnimTrees[i] = XAnimCreateTree(anims, (void* (*)(unsigned int))Hunk_AllocActorXAnimServer);
    g_scr_data.actorBackupXAnimTree = XAnimCreateTree(anims, (void* (*)(unsigned int))Hunk_AllocActorXAnimServer);
    for ( int i = 0; i < 8; ++i )
    {
...
#ifdef KISAK_SP
    G_PrintFastFileErrors((char*)"code_post_gfx");
    G_PrintFastFileErrors((char*)"common");
#else
...
// Decomp: CoDSP_rdBlackOps.map.c:6079784
...
// Decomp: CoDSP_rdBlackOps.map.c:6869041
...
// Decomp: CoDSP_rdBlackOps.map.c:6869242
...
// Decomp: SP behavior — no voting flow in single-player.
...
void __cdecl G_UpdateObjectiveToClients()
{
    ...
    for ( clientNum = 0; clientNum < level.maxclients; ++clientNum )
    {
        ...
        if ( ent->r.inuse )
        {
            ...
            ps = &ent->client->ps;
            team = G_GetClientTeam(clientNum);
...
// Decomp: CoDSP_rdBlackOps.map.c (TimedRadiusDamage @ 825C5378)
    ...
    int fireStarterClientNum)
{
    ...
    for ( i = 0; i < 512; ++i )
    {
        ...
        {
            g_timed_radius_damage[i].life = level.time + (int)(life * 1000.0f);
            g_timed_radius_damage[i].pos[0] = origin[0];
            g_timed_radius_damage[i].pos[1] = origin[1];
            g_timed_radius_damage[i].pos[2] = origin[2];
            g_timed_radius_damage[i].radius = radius;
            g_timed_radius_damage[i].radiusSqr = radius * radius;
            g_timed_radius_damage[i].damage = damage;
            g_timed_radius_damage[i].rate = (int)(rate * 1000.0f);
            g_timed_radius_damage[i].mod = MOD_EXPLOSIVE;
            g_timed_radius_damage[i].weapon = -1;
            g_timed_radius_damage[i].fireStarterClientNum = fireStarterClientNum;
            if ( i + 1 > g_timed_radius_damage_count )
                g_timed_radius_damage_count = i + 1;
            return;
        }
    }
}
...
// Decomp: CoDSP_rdBlackOps.map.c:6081715
...
#ifdef KISAK_SP
    {
        PROF_SCOPED("G_CheckEntityScriptSoundNotify");
        ent = g_entities;
        i = 0;
        while ( i < level.num_entities )
        {
            if ( ent->r.inuse )
                G_CheckEntityScriptSoundNotify(ent);
            ++i;
            ++ent;
        }
    }
...
void __cdecl G_RunFrameForEntity(gentity_s *ent)
{
    ...
    if ( ent->processedFrame != level.framenum )
    {
        ...
        if ( ent->scr_vehicle )
        {
            if ( (unsigned int)(ent->scr_vehicle - level.vehicles) >= MAX_VEHICLES
                && !Assert_MyHandler(
                            "C:\\projects_pc\\cod\\codsrc\\src\\game_mp\\g_main_mp.cpp",
                            3135,
                            0,
...
// Decomp: CoDSP_rdBlackOps.map.c:6082453
...
// Decomp: CoDSP_rdBlackOps.map.c:6080928
...
// Decomp: CoDSP_rdBlackOps.map.c:6080227
void __cdecl G_CheckReloadStatus()
{
    if ( level.changeLevelPending )
    {
        level.changeLevelPending = 0;
        G_ChangeLevel();
    }

    if ( level.nextMapExecuteTime && level.nextMapExecuteTime < level.time )
    {
        iassert(G_IsNextMapWaiting());
        level.nextMapExecuteTime = 0;

        if ( level.endMissionScreen )
        {
            level.endMissionScreen = 0;
            if ( level.cinematicMapTransition )
            {
                level.cinematicMapTransition = false;
                Cbuf_AddText(0, va("cinematic %s 0\n", level.nextMap));
            }
            else
            {
                SV_GameSendServerCommand(-1, SV_CMD_CAN_IGNORE, va("%c", '1'));
            }
        }
        else if ( level.loadgameContinuePending )
        {
            level.loadgameContinuePending = 0;
    ...
    if ( level.reloadDelayEndTime )
    {
        if ( (int)(level.reloadDelayEndTime - Sys_Milliseconds()) < 0 )
        {
            iassert(g_reloading && g_reloading->current.integer);
            level.reloadDelayEndTime = 0;
            Cbuf_AddText(0, "loadgame_continue\n");
        }
    }
}

// Decomp: CoDSP_rdBlackOps.map.c:825CA300
char *__cdecl G_ChangeLevel()
{
    if ( g_reloading && g_reloading->current.integer )
        return NULL;

    if ( level.missionFadeDuration )
    {
        if ( !level.endMissionScreen )
            SV_GameSendServerCommand(-1, SV_CMD_CAN_IGNORE, va("%c 0 %i\n", '0', level.missionFadeDuration + 1000));
        level.nextMapExecuteTime = level.time + 1000 + level.missionFadeDuration;
    }
    else
    {
        level.nextMapExecuteTime = level.time + 1000;
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (SP next-map / deferred loadgame queue)
bool __cdecl G_IsNextMapWaiting()
{
    return level.nextMap[0] != 0 || level.loadgameContinuePending != 0;
}

// Decomp: CoDSP_rdBlackOps.map.c (SP mission transition)
#ifdef KISAK_SP
void __cdecl G_LoadNextMap(int fromChangeLevel)
{
    ...
    if ( level.loadgameContinuePending )
    {
        level.loadgameContinuePending = 0;
        Cbuf_AddText(0, "loadgame_continue\n");
        return;
    ...
    if ( !fromChangeLevel )
        Com_PrintWarning(15, "G_LoadNextMap: no next map queued (SP)\n");
}
#else
void __cdecl G_LoadNextMap(int fromChangeLevel)
{
    (void)fromChangeLevel;
    if ( level.nextMap[0] )
        Cbuf_AddText(0, va("map %s\n", level.nextMap));
    else
        Cbuf_AddText(0, "map_rotate\n");
...
// Decomp: CoDSP_rdBlackOps.map.c (G_IsCoopVersusMode)
bool G_IsCoopVersusMode()
{
    return g_gametype && !I_stricmp(g_gametype->current.string, "vs");
}

// Decomp: CoDSP_rdBlackOps.map.c (G_RegisterCoopDvars)
const dvar_s *G_RegisterCoopDvars()
{
    _Dvar_RegisterInt("coop_player_num", 1, 1, 4, 0, "Coop player count");
    _Dvar_RegisterBool("coop_splitscreen", false, 0, "Coop splitscreen enabled");
    return _Dvar_RegisterString("coop_mode", "", 0, "Coop game mode");
}
