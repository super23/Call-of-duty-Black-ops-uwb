...
// Decomp: CoDSP_rdBlackOps.map.c (G_ParseHitLocDmgTable)
void __cdecl G_ParseHitLocDmgTable()
{
    ...
    // Decomp: CoDSP_rdBlackOps.map.c — verified code_post_gfx.ff (Unlinker): info/ai_lochit_dmgtable (MP: info/mp_lochit_dmgtable in common_mp.ff).
    if ( !BG_SP_TryLoadInfoStringFirst(
            "info/ai_lochit_dmgtable",
            ...
            &pszBuffer) )
    {
        Com_PrintWarning(
            16,
            "SP: missing hitloc damage table (info/ai_lochit_dmgtable) — using 1.0 multipliers (CoDSP_rdBlackOps.map.c)\n");
        return;
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (LookAtKiller)
...
LABEL_11:
    ...
    if ( !self->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 199, 0, "%s", "self->client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (G_MeansOfDeathFromScriptParam)
...
// Decomp: CoDSP_rdBlackOps.map.c (player_die)
                ...
                int psTimeOffset)
{
    ...
    SV_CheckThread();
    if ( !self->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 348, 0, "%s", "self->client") )
    {
        __debugbreak();
    }
        ...
        && (self->client->ps.otherFlags & 2) == 0 )
    {
        ...
        if ( (self->client->ps.eFlags & 0x300) != 0 )
        {
            if ( self->client->ps.viewlocked_entNum == 1023
                && !Assert_MyHandler(
                            "C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp",
                            390,
                            0,
                            "%s",
...
// Decomp: CoDSP_rdBlackOps.map.c (DeathGrenadeDrop)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetWeaponHitLocationMultiplier)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_DamageClient)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetWeaponIndexForEntity)
unsigned int __cdecl G_GetWeaponIndexForEntity(const gentity_s *ent)
{
    gclient_s *client;

    if ( !ent && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 544, 0, "%s", "ent") )
        __debugbreak();
    client = ent->client;
    if ( !client )
        return ent->s.weapon;
    ...
    if ( client->ps.viewlocked == PLAYERVIEWLOCK_NONE
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 558, 0, "%s", "ps->viewlocked") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (G_DamageActor)
                ...
                int timeOffset)
{
    ...
    if ( targ->takedamage && damage > 0 && targ->actor->Physics.bIsAlive )
    {
        ...
        if ( damage <= 0 )
            damage = 1;
        Scr_ActorDamage(targ, inflictor, attacker, damage, dflags, mod, weapon, point, dir, hitLoc, modelIndex, timeOffset);
    }
}

// Decomp: CoDSP_rdBlackOps.map.c (G_DamageVehicle)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_DamageNotify)
                ...
                char *weaponName)
{
    ...
    // Decomp: CoDSP_rdBlackOps.map.c G_DamageNotify @825AF750 — 9 notify params (no standalone partName slot).
    ...
    }
    {
        ...
        modNotify = mod;
        if ( (unsigned int)modNotify >= MOD_NUM )
            modNotify = MOD_UNKNOWN;
...
// Decomp: CoDSP_rdBlackOps.map.c (G_Damage)
                ...
                int timeOffset)
{
    ...
    {
        G_DamageActor(targ, inflictor, attacker, (float *)dir, (float *)point, damage, dFlags, mod, weapon, hitLoc, modelIndex, timeOffset);
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (CanDamage)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_FlashbangBlast)
void __cdecl G_FlashbangBlast(float *origin, float radius_max, float radius_min, gentity_s *attacker, team_t team)
{
    ...
    for ( i = 0; i < entListCount; ++i )
    {
        ...
        if ( !ent && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 1358, 0, "%s", "ent") )
            __debugbreak();
        FlashbangBlastEnt(ent, origin, radius_max, radius_min, attacker, team);
    }
}

// Decomp: CoDSP_rdBlackOps.map.c (GetEntListForRadius)
...
// Decomp: CoDSP_rdBlackOps.map.c (FlashbangBlastEnt)
...
// Decomp: CoDSP_rdBlackOps.map.c (EntDistToPoint)
...
// Decomp: CoDSP_rdBlackOps.map.c (AddScrTeamName)
...
// Decomp: CoDSP_rdBlackOps.map.c (CanEntityBeFlashbanged)
...
// Decomp: CoDSP_rdBlackOps.map.c (GetFlashbangViewPos)
...
// Decomp: CoDSP_rdBlackOps.map.c (GetFlashbangViewDirection)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_WithinDamageRadius)
bool __cdecl G_WithinDamageRadius(const float *damageOrigin, float radiusSquared, gentity_s *ent)
{
    float distSqrd;

    if ( !ent && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 1405, 0, "%s", "ent") )
        __debugbreak();
    distSqrd = G_GetRadiusDamageDistanceSquared(damageOrigin, ent);
    return radiusSquared > distSqrd;
}

// Decomp: CoDSP_rdBlackOps.map.c (G_GetRadiusDamageDistanceSquared)
double __cdecl G_GetRadiusDamageDistanceSquared(const float *damageOrigin, gentity_s *ent)
{
    int i;
    float v[3];

    if ( !ent && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\\g_combat_sp.cpp", 1373, 0, "%s", "ent") )
        __debugbreak();
    if ( ent->r.bmodel )
    {
        for ( i = 0; i < 3; ++i )
...
// Decomp: CoDSP_rdBlackOps.map.c (G_RadiusDamage)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetHitLocationString)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetHitLocationIndexFromString)
