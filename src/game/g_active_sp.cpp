...
// Decomp: CoDSP_rdBlackOps.map.c (P_DamageFeedback)
void __cdecl P_DamageFeedback(gentity_s *player)
{
    ...
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 56, 0, "%s", "client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientImpacts)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_DoTouchTriggers)
void __cdecl G_DoTouchTriggers(gentity_s *ent)
{
    ...
    if (!ent->client
        ...
        && !ent->scr_vehicle
        && !Assert_MyHandler(
            "C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp",
            241,
            0,
            "%s",
...
// Decomp: CoDSP_rdBlackOps.map.c (SpectatorThink)
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientInactivityTimer)
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientIntermissionThink)
...
// Decomp: CoDSP_rdBlackOps.map.c (NotifyGrenadePullback)
void __cdecl NotifyGrenadePullback(gentity_s *ent, unsigned int weaponIndex)
{
    char *tmp2;

    if ( !ent && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 507, 0, "%s", "ent") )
        __debugbreak();
    if ( !ent->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 508, 0, "%s", "ent->client") )
    {
        __debugbreak();
    }
    if ( !weaponIndex
...
// Decomp: CoDSP_rdBlackOps.map.c (HandleClientEvent)
void __cdecl HandleClientEvent(gclient_s *client, gentity_s *ent, int event, int eventParm)
{
    ...
    switch ( event )
    {
        case 9:
            if ( !ent->client
                && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 711, 0, "%s", "ent->client") )
            {
                __debugbreak();
            }
        ...
        case 22:
            if ( !ent->client
                && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 696, 0, "%s", "ent->client") )
            {
                __debugbreak();
            }
        ...
        case 23:
            if ( !ent->client
                && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 701, 0, "%s", "ent->client") )
            {
                __debugbreak();
            }
        ...
        case 41:
            if ( !ent->client
                && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 634, 0, "%s", "ent->client") )
            {
                __debugbreak();
            }
...
LABEL_6:
        ...
        case 91:
            if ( !ent->client
                && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 706, 0, "%s", "ent->client") )
            {
                __debugbreak();
            }
...
// Decomp: CoDSP_rdBlackOps.map.c (AttemptLiveGrenadePickup)
void __cdecl AttemptLiveGrenadePickup(gentity_s *clientEnt)
{
    ...
    if (!clientEnt
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 539, 0, "%s", "clientEnt"))
    {
        __debugbreak();
    }
    if (!clientEnt->client
...
int __thiscall EntHandle::entnum() const
{
    if ( (unsigned int)this->number - 1 >= 0x3FF
        && !Assert_MyHandler(
                    "c:\\projects_pc\\cod\\codsrc\\src\\game\\g_public_sp.h",
                    404,
                    0,
...
// Decomp: CoDSP_rdBlackOps.map.c (IsLiveGrenade)
bool __cdecl IsLiveGrenade(gentity_s *ent)
{
    ...
    if ( !weapDef
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 524, 0, "%s", "weapDef") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientEvents)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_SetLastServerTime)
void __cdecl G_SetLastServerTime(int clientNum, int lastServerTime)
{
    ...
    if ( !ent->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 817, 0, "%s", "ent->client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (G_SetClientContents)
void __cdecl G_SetClientContents(gentity_s *pEnt)
{
    if ( !pEnt->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 839, 0, "%s", "pEnt->client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientVehicleInteraction)
void __cdecl ClientVehicleInteraction(gentity_s *ent)
{
    ...
    client = ent->client;
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 921, 0, "%s", "client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientVehicleJumpOff)
void __cdecl ClientVehicleJumpOff(gentity_s *ent)
{
    ...
    client = ent->client;
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 982, 0, "%s", "client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (G_PlayerVehiclePositionAndBlend)
void __cdecl G_PlayerVehiclePositionAndBlend(gentity_s *ent, gentity_s *pTurretEnt)
{
    ...
    if ( !ci->infoValid
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 1073, 0, "%s", "ci->infoValid") )
    {
        __debugbreak();
    }
    ps = &ent->client->ps;
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientThink_real)
...
// Decomp: CoDSP_rdBlackOps.map.c (Player_WaterUpdate)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_PlayerStateToEntityStateExtrapolate)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_AddPlayerMantleBlockage)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_DrawServerAiming)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_BuildWeaponState)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_SaveWeaponState)
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientThink)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_RunClient)
void __cdecl G_RunClient(gentity_s *ent)
{
    float *origin;

    if ( !ent->client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 1753, 0, "%s", "ent->client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (IntermissionClientEndFrame)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_ClientCanSpectateTeam)
bool __cdecl G_ClientCanSpectateTeam(gclient_s *client, team_t team)
{
    return (client->sess.noSpectate & (1 << team)) == 0;
}

// Decomp: CoDSP_rdBlackOps.map.c (G_ClientCanSpectateTeamOrLocalPlayer)
...
// Decomp: CoDSP_rdBlackOps.map.c (SpectatorClientEndFrame)
...
// Decomp: CoDSP_rdBlackOps.map.c (GetFollowPlayerState)
int __cdecl GetFollowPlayerState(unsigned int clientNum, playerState_s *ps)
{
    ...
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2075, 0, "%s", "client") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (StuckInClient)
...
// Decomp: CoDSP_rdBlackOps.map.c (CM_CheckForTraps)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_PlayerController)
void __cdecl G_PlayerController(const gentity_s *self, int *partBits)
{
    ...
    if ( !ci->infoValid
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2291, 0, "%s", "ci->infoValid") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (G_UpdateClientInfo)
int __cdecl G_UpdateClientInfo(gentity_s *ent)
{
    ...
    client = ent->client;
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2322, 0, "%s", "client") )
    {
        __debugbreak();
    }
    ...
    ci = &level_bgs.clientinfo[clientNum];
    if ( !ci->infoValid
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2328, 0, "%s", "ci->infoValid") )
    {
        __debugbreak();
    }
    bChanged = 0;
...
// Decomp: CoDSP_rdBlackOps.map.c (ClientEndFrame)
void __cdecl ClientEndFrame(gentity_s *ent)
{
    ...
    client = ent->client;
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2442, 0, "%s", "client") )
    {
        __debugbreak();
    }
    if ( client->sess.connected == CON_DISCONNECTED
    ...
    // via ClientEndFrame (same pattern as CoDSP_rdBlackOps.map.c ClientEndFrame).
    if ( (client->flags & 4) != 0 )
        tmp1 = client->ps.pm_flags | 0x800;
    else
        tmp1 = client->ps.pm_flags & 0xFFFFF7FF;
    // Decomp: CoDSPBlackOps.map.c:3851345 (ClientEndFrame) — freezecontrolsallowlook uses client->flags bit 0x10.
    if ( (client->flags & 0x10) != 0 )
        tmp1 |= 0x10;
    else
        tmp1 &= ~0x10u;
    ...
    if ( client->sess.connected == CON_CONNECTED )
    {
        ...
        else if ( client->ps.clientNum == ent->s.number )
        {
            ...
            client->ps.satelliteTypeEnabled = level_tail.teamHasSatellite[client->sess.cs.team];
            if ( client->hasSatellite )
                client->ps.satelliteTypeEnabled = client->hasSatellite;
            client->ps.spyplaneTypeEnabled = level_tail.teamHasSpyplane[client->sess.cs.team];
            ...
            if ( (ent->s.lerp.eFlags & 0x4000) != 0 && (ent->client->ps.eFlags2 & 0x10000000) != 0 )
            {
                ...
                if ( !veh
                    && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2638, 0, "%s", "veh") )
                {
                    __debugbreak();
                }
                info = BG_GetVehicleInfo(veh->s.vehicleState.vehicleInfoIndex);
                if ( !info
                    && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2640, 0, "%s", "info") )
                {
                    __debugbreak();
                }
...
// Decomp: CoDSP_rdBlackOps.map.c (G_AddClientKnife)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetPlayer)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_PlayerEvent)
void __cdecl G_PlayerEvent(int clientNum, int event)
{
    ...
    client = g_entities[clientNum].client;
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_active_sp.cpp", 2773, 0, "%s", "client") )
    {
        __debugbreak();
    }
