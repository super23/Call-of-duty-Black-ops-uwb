...
#include <game/sentient_script_cmd.h>
...
// SP-only player entity methods (g_client_script_cmd_sp.cpp / KisakBlackSP).
// Retail CoDSP_rd.map registers script keys here; CoDMP_rd.map does not export most of them.
// Do NOT copy new entries into MP g_client_script_cmd without verifying CoDMP_rd.map.
...
const BuiltinMethodDef methods[] =
{
  ...
  { "reviveplayer", &PlayerCmd_RevivePlayer, 0 },
  { "startrevive", &PlayerCmd_StartRevive, 0 },
  { "stoprevive", &PlayerCmd_StopRevive, 0 },
  ...
  { "initdec20terminal", &PlayerCmd_InitDec20Terminal, 0 },
  { "attachtodec20terminal", &PlayerCmd_AttachToDec20Terminal, 0 },
  { "detachfromdec20terminal", &PlayerCmd_DetachDec20Terminal, 0 },
  { "detachdec20terminal", &PlayerCmd_DetachDec20Terminal, 0 },
  { "uploadscore", &PlayerCmd_UploadScore, 0 },
  { "throwdownweapon", &PlayerCmd_ThrowDownWeapon, 0 },
  { "islookingat", &PlayerCmd_IsLookingAt, 0 },
  { "updateweaponoptions", &PlayerCmd_updateWeaponOptions, 0 },
  { "isweaponoverheating", &PlayerCmd_isWeaponOverheating, 0 },
  { "setadswidthandlerp", &PlayerCmd_SetAdsWidthAndLerp, 0 },
  { "resetadswidthandlerp", &PlayerCmd_ResetAdsWidthAndLerp, 0 },
  { "startcameratween", &PlayerCmd_startCameraTween, 0 },
  { "setviewlockent", &PlayerCmd_setViewLockEnt, 0 },
  { "setplayerviewratescale", &PlayerCmd_SetPlayerViewRateScale, 0 },
  { "resetplayerviewratescale", &PlayerCmd_ResetPlayerViewRateScale, 0 },
  { "clearviewlockent", &PlayerCmd_clearViewLockEnt, 0 },
  { "getcamerapos", &PlayerCmd_getCameraPos, 0 },
  { "getplayercamerapos", &PlayerCmd_getCameraPos, 0 },
  // Retail key: getplayerviewheight (CoDSP_rd.map @ g_client_script_cmd.obj). Decomp name: getviewheight.
  { "getplayerviewheight", &PlayerCmd_getViewHeight, 0 },
  { "getviewheight", &PlayerCmd_getViewHeight, 0 },
  { "getnormalizedmovement", &PlayerCmd_getNormalizedMovement, 0 },
  { "getnormalizedcameramovement", &PlayerCmd_getNormalizedCameraMovement, 0 },
  { "usebuttonheld", &PlayerCmd_useButtonHeld, 0 },
  { "sprintbuttonpressed", &PlayerCmd_sprintButtonPressed, 0 },
  { "gasbuttonpressed", &PlayerCmd_gasButtonPressed, 0 },
  { "reloadbuttonpressed", &PlayerCmd_reloadButtonPressed, 0 },
  { "reversebuttonpressed", &PlayerCmd_reverseButtonPressed, 0 },
  { "checkbuttonpressed", &PlayerCmd_CheckButtonPressed, 0 },
  { "notifyoncommand", &PlayerCmd_notifyOnCommand, 0 },
  { "openmainmenu", &PlayerCmd_OpenMainMenu, 0 },
  { "playerknockback", &PlayerCmd_PlayerKnockback, 0 },
  { "closemainmenu", &PlayerCmd_CloseMainMenu, 0 },
  { "setautopickup", &ScrCmd_SetAutoPickup, 0 },
  { "getweaponrenderoptions", &PlayerCmd_GetWeaponRenderOptions, 0 },
  { "disableweaponfire", &PlayerCmd_DisableWeaponFire, 0 },
  { "enableweaponreload", &PlayerCmd_EnableWeaponReload, 0 },
  { "disableweaponreload", &PlayerCmd_DisableWeaponReload, 0 },
  { "enableweaponfire", &PlayerCmd_EnableWeaponFire, 0 },
  ...
  { "cleardamageindicator", &PlayerCmd_ClearDamageIndicator, 0 },
  { "undolaststand", &PlayerCmd_UndoLastStand, 0 },
  { "getcurrentarcadehighscore", &PlayerCmd_GetCurrentArcadeHighScore, 0 },
  { "getscoremultiplier", &PlayerCmd_GetScoreMultiplier, 0 },
  { "setscoremultiplier", &PlayerCmd_SetScoreMultiplier, 0 },
  { "setlowready", &PlayerCmd_SetLowReady, 0 },
  { "setloweredweapon", &PlayerCmd_SetLoweredWeapon, 0 },
  { "setscripthintstring", &PlayerCmd_SetScriptHintString, 0 },
  { "allowpickupweapons", &PlayerCmd_AllowPickupWeapons, 0 }
};



// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setBlockWeaponPickup)
void __cdecl PlayerCmd_setBlockWeaponPickup(scr_entref_t entref)
{
    ...
    else
    {
        if ( entref.entnum >= 0x400u
            && !Assert_MyHandler(
                        "C:\\projects_pc\\cod\\codsrc\\src\\game\\g_client_script_cmd_sp.cpp",
                        156,
                        0,
                        "%s",
...
// Decomp: CoDSP_rdBlackOps.map.c (BG_PlayerWeaponSetBlockPickupWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_giveWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_InitializeAmmo)
...
// Decomp: CoDSP_rdBlackOps.map.c (G_GetNeededStartAmmo)
int __cdecl G_GetNeededStartAmmo(gentity_s *pSelf, unsigned int weaponIndex)
{
    ...
    if ( !pSelf
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\game\\g_client_script_cmd_sp.cpp", 67, 0, "%s", "pSelf") )
    {
        __debugbreak();
    }
    if ( !pSelf->client
...
// Decomp: CoDSP_rdBlackOps.map.c (BG_GetStartAmmo)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_giveNextBaseWeaponDEVONLY)
...
// Decomp: CoDSP_rdBlackOps.map.c (WeaponComponentListAddAllPoints)
...
// Decomp: CoDSP_rdBlackOps.map.c (WeaponComponentListToName)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_giveWeaponNextAttachmentDEVONLY)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_calcWeaponOptions)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetStatFromPlayer)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_calcPlayerOptions)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setPlayerRenderOptions)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_NextPlayerRenderOption)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetHighlighted)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsThrowingGrenade)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_takeWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_takeAllWeapons)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getCurrentWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (8259C578 PlayerCmd_getCurrentWeaponClipAmmo)
void __cdecl PlayerCmd_getCurrentWeaponClipAmmo(scr_entref_t entref)
{
    ...
    if ( !pSelf->client->ps.weapon )
        Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
    else
        Scr_AddInt(BG_GetAmmoInClip(&pSelf->client->ps, pSelf->client->ps.weapon), SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (ClientPlaying)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getCurrentOffhand)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setOffhandSecondaryClass)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setOffhandPrimaryClass)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getOffhandSecondaryClass)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_hasWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_switchToWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_switchToOffhand)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getLockOnRadius)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getLockOnSpeed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_giveStartAmmo)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_giveMaxAmmo)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getFractionStartAmmo)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getFractionMaxAmmo)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setOrigin)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetVelocity)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setAngles)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_getAngles)
...
// Decomp: g_client_script_cmd.cpp:1285
void __cdecl PlayerCmd_CameraActivate(scr_entref_t entref)
{
    ...
    activate = Scr_GetInt(0, SCRIPTINSTANCE_SERVER) != 0;
    Com_sprintf(svcmd, sizeof(svcmd), "%c %i", 41, activate);
    SV_GameSendServerCommand(pSelf->s.clientNum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: g_client_script_cmd.cpp:1313
void __cdecl PlayerCmd_CameraSetPosition(scr_entref_t entref)
{
    ...
    if ( Scr_GetType(0, SCRIPTINSTANCE_SERVER) == 4 )
    {
        ...
        if ( Scr_GetNumParam(SCRIPTINSTANCE_SERVER) > 1 && Scr_GetType(1u, SCRIPTINSTANCE_SERVER) == 4 )
    ...
    {
        iassert(Scr_GetType(0, SCRIPTINSTANCE_SERVER) == 1 && Scr_GetPointerType(0, SCRIPTINSTANCE_SERVER) == 19);
        ent = Scr_GetEntity(0);
        Com_sprintf(svcmd, sizeof(svcmd), "%c %i %i", 41, 4, ent->s.number);
        SV_GameSendServerCommand(pSelf->s.clientNum, SV_CMD_RELIABLE, svcmd);
    }
}

// Decomp: g_client_script_cmd.cpp:1355
void __cdecl PlayerCmd_CameraSetLookAt(scr_entref_t entref)
{
    ...
    if ( Scr_GetNumParam(SCRIPTINSTANCE_SERVER) )
    {
        ...
        {
            Scr_GetVector(0, origin, SCRIPTINSTANCE_SERVER);
            Com_sprintf(svcmd, sizeof(svcmd), "%c %i %f %f %f", 41, 8, origin[0], origin[1], origin[2]);
            SV_GameSendServerCommand(pSelf->s.clientNum, SV_CMD_RELIABLE, svcmd);
        }
        else
        {
            iassert(Scr_GetType(0, SCRIPTINSTANCE_SERVER) == 1);
            iassert(Scr_GetPointerType(0, SCRIPTINSTANCE_SERVER) == 19);
            ent = Scr_GetEntity(0);
            Com_sprintf(svcmd, sizeof(svcmd), "%c %i %i", 41, 16, ent->s.number);
            SV_GameSendServerCommand(pSelf->s.clientNum, SV_CMD_RELIABLE, svcmd);
        }
    }
    else
    {
        Com_sprintf(svcmd, sizeof(svcmd), "%c %i", 41, 32);
        SV_GameSendServerCommand(pSelf->s.clientNum, SV_CMD_RELIABLE, svcmd);
    }
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_Revive)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A9318 PlayerCmd_RevivePlayer)
void __cdecl PlayerCmd_RevivePlayer(scr_entref_t entref)
{
    ...
    clientState->lastDamageTime = 0;
    clientState->needsRevive = 0;
    pSelf->health = pSelf->client->sess.maxHealth;
    pSelf->maxHealth = pSelf->client->sess.maxHealth;
    pSelf->client->damage_blood = 0;
    pSelf->client->ps.stats[0] = pSelf->health;
    pSelf->client->damage_fromWorld = 0;
    pSelf->client->sess.sessionState = SESS_STATE_PLAYING;
    pSelf->client->flags &= ~CLIENT_FLAG_SCRIPT_SPECTATOR;
    pSelf->client->flags &= ~4u;
    pSelf->client->flags &= ~0x10u;
}

// Decomp: CoDSP_rdBlackOps.map.c (825A9B58 PlayerCmd_StartRevive)
void __cdecl PlayerCmd_StartRevive(scr_entref_t entref)
{
    ...
    pSelf->client->revive = 1;
    pSelf->client->ps.pm_type = 8;
    clientState = G_GetClientState(pSelf->s.number);
    clientState->needsRevive = 1;
}

// Decomp: CoDSP_rdBlackOps.map.c (825A9C58 PlayerCmd_StopRevive)
void __cdecl PlayerCmd_StopRevive(scr_entref_t entref)
{
    ...
    pSelf->client->revive = 0;
    pSelf->client->ps.pm_type = 7;
    clientState = G_GetClientState(pSelf->s.number);
    clientState->needsRevive = 0;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_useButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ChangeSeatButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_attackButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_actionSlotOneButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_actionSlotTwoButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_actionSlotThreeButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_actionSlotFourButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_adsButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_throwButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_meleeButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_jumpButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_fragButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_secondaryOffhandButtonPressed)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_buttonPressedDEVONLY)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_playerADS)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_pingPlayer)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetViewmodel)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ShowViewmodel @ 825A1300)
void __cdecl PlayerCmd_ShowViewmodel(scr_entref_t entref)
{
    ...
    _snprintf(svcmd, 0x40u, "%c", 123);
    svcmd[63] = 0;
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_HideViewmodel @ 825A13E8)
void __cdecl PlayerCmd_HideViewmodel(scr_entref_t entref)
{
    ...
    ps = &pSelf->client->ps;
    ws = (weaponstate_t)ps->weaponstate;
    if ( ws == WEAPON_RELOADING_INTERUPT
        || ws == WEAPON_RELOAD_START_INTERUPT
        || ws == WEAPON_RELOAD_QUICK
        || ws == WEAPON_RELOAD_END
        || ws == WEAPON_RELOAD_START
        || ws == WEAPON_RELOAD_QUICK_EMPTY
        || ws == WEAPON_MELEE_INIT )
    {
        BG_AddPredictableEventToPlayerstate(2u, ps->weaponstate, ps);
    }
    PM_ResetWeaponState(ps);
    _snprintf(svcmd, 0x40u, "%c", 125);
    svcmd[63] = 0;
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetViewmodel)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setSpawnWeapon)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_dropItem)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_dropScavengerItem)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_hasAGrenadePulledBack)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BotSetScriptGoal)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BotClearScriptGoal)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BotSetScriptEnemy)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BotClearScriptEnemy)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BotSetAttacker)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BotPressUseButton)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_finishPlayerDamage)
...
// Decomp: CoDSP_rdBlackOps.map.c (IsBulletImpactMOD)
...
// Decomp: CoDSP_rdBlackOps.map.c (ForceGrenadeThrow)
...
// Decomp: CoDSP_rdBlackOps.map.c (ApplyKnockBack)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_Suicide)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsSplitscreen)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsPlayerOnSameMachine)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsInVehicle)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsRemoteControlling)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetVehicleOccupied)
...
// Decomp: g_client_script_cmd.cpp:2416
void __cdecl PlayerCmd_OpenMenu(scr_entref_t entref)
{
    ...
    {
        menuName = Scr_GetString(0, SCRIPTINSTANCE_SERVER);
        iMenuIndex = GScr_GetScriptMenuIndex(menuName);
        iassert(iMenuIndex < 32);
        Com_sprintf(svcmd, sizeof(svcmd), "%c %i", 116, iMenuIndex);
        SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
        opened = 1;
    }
    Scr_AddInt(opened, SCRIPTINSTANCE_SERVER);
}

// Decomp: g_client_script_cmd.cpp:2645
void __cdecl PlayerCmd_OpenMenuNoMouse(scr_entref_t entref)
{
    ...
    if ( pSelf->client->sess.connected == CON_CONNECTED )
    {
        ...
        iMenuIndex = GScr_GetScriptMenuIndex(menuName);
        iassert(iMenuIndex < 32);
        Com_sprintf(svcmd, sizeof(svcmd), "%c %i 1", 116, iMenuIndex);
        SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
        opened = 1;
    }
    Scr_AddInt(opened, SCRIPTINSTANCE_SERVER);
...
// Decomp: g_client_script_cmd.cpp:2673
void __cdecl PlayerCmd_CloseMenu(scr_entref_t entref)
{
    ...
    Com_sprintf(svcmd, sizeof(svcmd), "%c", 117);
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: g_client_script_cmd.cpp:2722
void __cdecl PlayerCmd_CloseInGameMenu(scr_entref_t entref)
{
    ...
    Com_sprintf(svcmd, sizeof(svcmd), "%c", 76);
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetWeaponAmmoClip)
...
// Decomp: CoDSP_rdBlackOps.map.c (SetWeaponAmmoClipDualWield)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetWeaponAmmoStock)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetCurrentSpectatorClient)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetWeaponAmmoClip)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetWeaponAmmoStock)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AnyAmmoForWeaponModes)
...
// Decomp: CoDSP_rdBlackOps.map.c (iclientprintln)
...
// Decomp: CoDSP_rdBlackOps.map.c (iclientprintlnbold)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_spawn)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_setEnterTime)
...
// Decomp: CoDSP_rdBlackOps.map.c (BodyEnd)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ClonePlayer)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetClientDvar)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetClientUIVisibilityFlag)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetClientDvars)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_PlayClientAirstrike)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_PlayClientNapalm)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsTalking)
...
// Decomp: g_client_script_cmd.cpp:2821
...
// Decomp: CoDMPServer.c:380203 (MP retail; not in SP g_client_script_cmd.cpp)
void __cdecl PlayerCmd_AreControlsFrozen(scr_entref_t entref)
{
    ...
    Scr_AddInt((pSelf->client->flags & 4) != 0, SCRIPTINSTANCE_SERVER);
}

// Decomp: g_client_script_cmd.cpp:2800
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DisableUsability)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableUsability)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DisableWeapons)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableWeapons)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DisableDeathStreak)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableDeathStreak)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DeathStreakActive)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DisableOffhandWeapons)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableOffhandWeapons)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DisableWeaponCycling)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableWeaponCycling)
...
// Decomp: CoDSP_rdBlackOps.map.c (ScrCmd_PlayLocalSound)
...
// Decomp: CoDSP_rdBlackOps.map.c (ScrCmd_StopLocalSound)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SayAll)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SayTeam)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowADS)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowJump)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowSprint)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowStand)
void __cdecl PlayerCmd_AllowStand(scr_entref_t entref)
{
    ...
    else
    {
        if ( entref.entnum >= 0x400u
            && !Assert_MyHandler(
                        "C:\\projects_pc\\cod\\codsrc\\src\\game\\g_client_script_cmd.cpp",
                        0x916u,
                        0,
                        "%s",
                        "entref.entnum < MAX_GENTITIES") )
    ...
    if ( Scr_GetInt(0, SCRIPTINSTANCE_SERVER) )
        pSelf->client->ps.pm_flags &= ~0x400000u;
    else
        pSelf->client->ps.pm_flags |= 0x400000u;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowCrouch)
void __cdecl PlayerCmd_AllowCrouch(scr_entref_t entref)
{
    ...
    if ( Scr_GetInt(0, SCRIPTINSTANCE_SERVER) )
        pSelf->client->ps.pm_flags &= ~0x800000u;
    else
        pSelf->client->ps.pm_flags |= 0x800000u;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowProne)
void __cdecl PlayerCmd_AllowProne(scr_entref_t entref)
{
    ...
    if ( Scr_GetInt(0, SCRIPTINSTANCE_SERVER) )
        pSelf->client->ps.pm_flags &= ~0x1000000u;
    else
        pSelf->client->ps.pm_flags |= 0x1000000u;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowMelee)
void __cdecl PlayerCmd_AllowMelee(scr_entref_t entref)
{
    ...
    if ( Scr_GetInt(0, SCRIPTINSTANCE_SERVER) )
        pSelf->client->ps.pm_flags &= ~0x8000000u;
    else
        pSelf->client->ps.pm_flags |= 0x8000000u;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowLean ~825A18B0)
// Map: CoDSP_rd.map VA 0x825A18B0 (g_client_script_cmd.obj)
void __cdecl PlayerCmd_AllowLean(scr_entref_t entref)
{
    ...
    if ( Scr_GetInt(0, SCRIPTINSTANCE_SERVER) )
        pSelf->client->ps.pm_flags &= ~0x2000000u;
    else
        pSelf->client->ps.pm_flags |= 0x2000000u;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableHealthShield ~825A45A8)
// Map: CoDSP_rd.map VA 0x825A45A8 (g_client_script_cmd.obj)
void __cdecl PlayerCmd_EnableHealthShield(scr_entref_t entref)
{
    ...
    *(unsigned char *)((char *)pSelf->client + 7381) = Scr_GetInt(0, SCRIPTINSTANCE_SERVER) != 0;
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_UploadScore ~825A71C8)
// Map: CoDSP_rd.map VA 0x825A71C8 (g_client_script_cmd.obj)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ThrowDownWeapon ~825A7EB0)
// Map: CoDSP_rd.map VA 0x825A7EB0 (g_client_script_cmd.obj)
void __cdecl PlayerCmd_ThrowDownWeapon(scr_entref_t entref)
{
    ...
    if ( weaponIndex )
    {
        throwHeight = Scr_GetFloat(1u, SCRIPTINSTANCE_SERVER);
        throwDistance = Scr_GetFloat(2u, SCRIPTINSTANCE_SERVER);
        if ( Scr_GetNumParam(SCRIPTINSTANCE_SERVER) <= 3 )
            tagName = scr_const.tag_inhand;
        else
            tagName = Scr_GetConstLowercaseString(3u, SCRIPTINSTANCE_SERVER);
        weaponModel = BG_GetPlayerWeaponModel(&pSelf->client->ps, weaponIndex);
        ThrowDownWeapon(pSelf, weaponIndex, weaponModel, tagName, throwHeight, throwDistance);
    }
    else
    {
        item = G_FindItem((char *)weaponName);
        if ( item )
            Drop_Item(pSelf, item, 0.0, 0);
    }
}

// Decomp: CoDSP_rdBlackOps.map.c (ScrCmd_IsLookingAt ~825A3080)
// Map: CoDSP_rd.map VA 0x825A3080 (g_client_script_cmd.obj)
void __cdecl PlayerCmd_IsLookingAt(scr_entref_t entref)
{
    ...
    lookAtEnt = (EntHandle *)((char *)pSelf->client + 7296);
    targetEnt = Scr_GetEntity(0);
    if ( lookAtEnt->isDefined() && lookAtEnt->ent() == targetEnt )
        Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
    else
        Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetSprintDuration)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetSprintCooldown)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetSpreadOverride)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ResetSpreadOverride)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_AllowSpectateTeam)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetGuid)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetXuid)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsHost)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_IsLocalToHost)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationTypeSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationAirstrikeSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationMortarSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationArtillerySelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationComlinkSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_BeginLocationNapalmSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EndLocationSelection)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetActionSlot)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetWeaponsList)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetWeaponsListPrimaries)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetPerk)
...
// Decomp: CoDSP_rdBlackOps.map.c (BG_SetPerk)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_HasPerk)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_GetPerks)
void __cdecl PlayerCmd_GetPerks(scr_entref_t entref)
{
    ...
    Scr_MakeArray(SCRIPTINSTANCE_SERVER);
    for ( perkIndex = 0; perkIndex < BG_PERK_COUNT; ++perkIndex )
    {
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_UnsetPerk)
...
// Decomp: CoDSP_rdBlackOps.map.c (BG_UnsetPerk)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ClearPerks)
...
// Decomp: CoDSP_rdBlackOps.map.c (BG_ClearPerks)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_EnableInvulnerability)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_DisableInvulnerability)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetRank)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_SetLastStandPrevWeap)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ClientSysSetState)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_WeaponLockStart)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_WeaponLockFinalize)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_WeaponLockFree)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_WeaponLockTargetTooClose)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_WeaponLockNoClearance)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_ReLinkToTurret)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_CarryTurret)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_StopCarryTurret)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_LinkGuidedMissileCamera)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_UnlinkGuidedMissileCamera)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_isCarryingTurret)
...
// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_CanPlayerPlaceTurret)
...
// Decomp: g_client_script_cmd.cpp:2742
void __cdecl PlayerCmd_InitDec20Terminal(scr_entref_t entref)
{
    ...
    Com_sprintf(svcmd, sizeof(svcmd), "%c", 'I');
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: g_client_script_cmd.cpp:2761
void __cdecl PlayerCmd_AttachToDec20Terminal(scr_entref_t entref)
{
    ...
    Dvar_SetIntByName("dec20_inuse", 1);
    Com_sprintf(svcmd, sizeof(svcmd), "%c", 'D');
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: g_client_script_cmd.cpp:2781
void __cdecl PlayerCmd_DetachDec20Terminal(scr_entref_t entref)
{
    ...
    Dvar_SetIntByName("dec20_inuse", 0);
    Com_sprintf(svcmd, sizeof(svcmd), "%c", 'E');
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: CoDSP_rdBlackOps.map.c (GetPlayerScore ~7053302)
static int __cdecl GetPlayerScore(unsigned __int64 xuid, int missionIndex)
{
    (void)xuid;
    if ( missionIndex < 0 || missionIndex > 32 )
        return 0;
    return 0;
}

// Decomp: CoDSP_rdBlackOps.map.c (8259B9B0 PlayerCmd_updateWeaponOptions)
void __cdecl PlayerCmd_updateWeaponOptions(scr_entref_t entref)
{
    ...
    weaponName = Scr_GetString(0, SCRIPTINSTANCE_SERVER);
    weaponOptions = Scr_GetInt(1, SCRIPTINSTANCE_SERVER);
    weaponIndex = G_GetWeaponIndexForName(weaponName);
    Scr_VerifyWeaponIndex(weaponIndex, weaponName);
    if ( BG_PlayerHasWeapon(&pSelf->client->ps, weaponIndex) )
        G_UpdatePlayerWeaponOptions(&pSelf->client->ps, weaponIndex, weaponOptions);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259D868 PlayerCmd_isWeaponOverheating)
void __cdecl PlayerCmd_isWeaponOverheating(scr_entref_t entref)
{
    const char *fmtMsg;
    gentity_s *pSelf;
    playerState_s *ps;
    int returnHeatAsFloat;
    gentity_s *vehEnt;
    ...
    if ( Scr_GetNumParam(SCRIPTINSTANCE_SERVER) )
        returnHeatAsFloat = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    ps = &pSelf->client->ps;
    if ( (ps->eFlags & 0x4000) == 0 )
    {
        if ( (ps->pm_flags & 0x300) != 0 )
        {
            gentity_s *turretEnt = &g_entities[ps->viewlocked_entNum];
            if ( turretEnt->pTurretInfo )
                Scr_AddBool(*(unsigned char *)((char *)turretEnt->pTurretInfo + 156), SCRIPTINSTANCE_SERVER);
            else
                Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
            return;
        }
        if ( BG_PlayerWeaponOverheating(ps, ps->weapon) )
            Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
        else
            Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
        return;
    }
    vehEnt = &g_entities[ps->viewlocked_entNum];
    info = BG_GetVehicleInfo(vehEnt->s.vehicleState.vehicleInfoIndex);
    gunIndex = -1;
    if ( ps->vehiclePos >= 1 && ps->vehiclePos <= 4 )
        gunIndex = ps->vehiclePos - 1;
    if ( gunIndex >= 0 )
    {
        ...
        if ( returnHeatAsFloat )
            Scr_AddFloat(veh->gunnerTurrets[gunIndex].heatVal, SCRIPTINSTANCE_SERVER);
        else
            Scr_AddBool(veh->gunnerTurrets[gunIndex].overheating != 0, SCRIPTINSTANCE_SERVER);
        return;
    }
    if ( !vehEnt->active )
    {
        if ( info->driverControlledGunPos >= 0 )
            gunIndex = info->driverControlledGunPos;
    }
    veh = vehEnt->scr_vehicle;
    if ( gunIndex >= 0 && veh )
    {
        if ( returnHeatAsFloat )
            Scr_AddFloat(veh->turret.heatVal, SCRIPTINSTANCE_SERVER);
        else
            Scr_AddBool(veh->turret.overheating != 0, SCRIPTINSTANCE_SERVER);
        return;
    }
    Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259DF60 PlayerCmd_SetAdsWidthAndLerp)
void __cdecl PlayerCmd_SetAdsWidthAndLerp(scr_entref_t entref)
{
    ...
    width = Scr_GetFloat(0, SCRIPTINSTANCE_SERVER);
    lerp = Scr_GetFloat(1, SCRIPTINSTANCE_SERVER);
    AimAssist_SetAdsWidthAndLerp(pSelf->client->ps.clientNum, width, lerp);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259E068 PlayerCmd_ResetAdsWidthAndLerp)
void __cdecl PlayerCmd_ResetAdsWidthAndLerp(scr_entref_t entref)
{
    ...
    AimAssist_ResetAdsWidthAndLerp(pSelf->client->ps.clientNum);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259E148 PlayerCmd_startCameraTween)
void __cdecl PlayerCmd_startCameraTween(scr_entref_t entref)
{
    ...
    duration = Scr_GetFloat(0, SCRIPTINSTANCE_SERVER);
    tent = G_TempEntity(pSelf->r.currentOrigin, EV_START_CAMERA_TWEEN);
    AssignToSmallerType<unsigned short>(&tent->s.eventParm, (unsigned short)(int)(duration * 1000.0f));
}

// Decomp: CoDSP_rdBlackOps.map.c (8259E978 PlayerCmd_setViewLockEnt)
void __cdecl PlayerCmd_setViewLockEnt(scr_entref_t entref)
{
    ...
    ent = Scr_GetEntity(0);
    Dvar_SetInt((dvar_s *)player_viewLockEnt, ent->s.number);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259EA60 PlayerCmd_SetPlayerViewRateScale)
void __cdecl PlayerCmd_SetPlayerViewRateScale(scr_entref_t entref)
{
    float viewRateScale;

    (void)entref;
    iassert(Scr_GetNumParam(SCRIPTINSTANCE_SERVER) == 1);
    viewRateScale = Scr_GetFloat(0, SCRIPTINSTANCE_SERVER);
    iassert(viewRateScale > 0.0f);
    Dvar_SetFloat((dvar_s *)player_viewRateScale, viewRateScale);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259EB40 PlayerCmd_ResetPlayerViewRateScale)
void __cdecl PlayerCmd_ResetPlayerViewRateScale(scr_entref_t entref)
{
    (void)entref;
    Dvar_SetFloat((dvar_s *)player_viewRateScale, 0.0f);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259EB78 PlayerCmd_clearViewLockEnt)
void __cdecl PlayerCmd_clearViewLockEnt(scr_entref_t entref)
{
    ...
    Dvar_SetInt((dvar_s *)player_viewLockEnt, 0);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259EC50 PlayerCmd_getCameraPos)
void __cdecl PlayerCmd_getCameraPos(scr_entref_t entref)
{
    ...
    G_GetPlayerViewOrigin(&pSelf->client->ps, pos);
    Scr_AddVector(pos, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259EE28 PlayerCmd_getNormalizedMovement)
void __cdecl PlayerCmd_getNormalizedMovement(scr_entref_t entref)
{
    ...
    move[0] = (float)pSelf->client->sess.cmd.forwardmove / 127.0f;
    move[1] = (float)pSelf->client->sess.cmd.rightmove / 127.0f;
    move[2] = 0.0f;
    Scr_AddVector(move, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259EF88 PlayerCmd_getNormalizedCameraMovement)
void __cdecl PlayerCmd_getNormalizedCameraMovement(scr_entref_t entref)
{
    ...
    move[0] = (float)pSelf->client->sess.cmd.pitchmove / -127.0f;
    move[1] = (float)pSelf->client->sess.cmd.yawmove / -127.0f;
    move[2] = 0.0f;
    Scr_AddVector(move, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259ED48 PlayerCmd_getViewHeight)
void __cdecl PlayerCmd_getViewHeight(scr_entref_t entref)
{
    ...
    Scr_AddFloat(pSelf->client->ps.viewHeightCurrent, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259F278 PlayerCmd_useButtonHeld)
void __cdecl PlayerCmd_useButtonHeld(scr_entref_t entref)
{
    ...
    Scr_AddBool(
        level.time - pSelf->client->useHoldTime >= g_useholdtime->current.integer,
        SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259DD88 PlayerCmd_GetVelocity)
void __cdecl PlayerCmd_GetVelocity(scr_entref_t entref)
{
    ...
    Scr_AddVector(pSelf->client->ps.velocity, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (PlayerCmd_reverseButtonPressed)
void __cdecl PlayerCmd_reverseButtonPressed(scr_entref_t entref)
{
    ...
    if ( pSelf->client->button_bitsSinceLastFrame.testBit(0x17u)
        || pSelf->client->button_bits.testBit(0x17u) )
        Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
    else
        Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259F898 PlayerCmd_sprintButtonPressed)
void __cdecl PlayerCmd_sprintButtonPressed(scr_entref_t entref)
{
    ...
    if ( pSelf->client->button_bitsSinceLastFrame.testBit(1u)
        || pSelf->client->button_bits.testBit(1u) )
        Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
    else
        Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259F9C0 PlayerCmd_gasButtonPressed)
void __cdecl PlayerCmd_gasButtonPressed(scr_entref_t entref)
{
    ...
    if ( pSelf->client->button_bitsSinceLastFrame.testBit(0x16u)
        || pSelf->client->button_bits.testBit(0x16u) )
        Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
    else
        Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (8259FAE8 PlayerCmd_reloadButtonPressed)
void __cdecl PlayerCmd_reloadButtonPressed(scr_entref_t entref)
{
    ...
    if ( pSelf->client->button_bitsSinceLastFrame.testBit(4u)
        || pSelf->client->button_bits.testBit(4u) )
        Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
    else
        Scr_AddInt(0, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A00B0 PlayerCmd_CheckButtonPressed)
void __cdecl PlayerCmd_CheckButtonPressed(scr_entref_t entref)
{
    ...
    else
    {
        ...
        if ( !button || !*button )
            Scr_ParamError(0, "usage: buttonPressed(<button name>)", SCRIPTINSTANCE_SERVER);
        localClientNum = Com_ControllerIndex_GetLocalClientNum(0);
        Scr_AddInt(CL_IsKeyPressed(localClientNum, button) != 0, SCRIPTINSTANCE_SERVER);
    }
}

// Decomp: CoDSP_rdBlackOps.map.c (825A0960 PlayerCmd_notifyOnCommand)
void __cdecl PlayerCmd_notifyOnCommand(scr_entref_t entref)
{
    (void)entref;
    Scr_Error("USAGE:  notifyOnCommand() is not a valid command for coop\n", SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A1B48 PlayerCmd_OpenMainMenu)
void __cdecl PlayerCmd_OpenMainMenu(scr_entref_t entref)
{
    ...
    iMenuIndex = GScr_GetScriptMenuIndex(menuName);
    iassert(iMenuIndex < 32);
    Com_sprintf(svcmd, sizeof(svcmd), "%c %i", 'b', iMenuIndex);
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
    Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A2290 PlayerCmd_PlayerKnockback)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A2718 PlayerCmd_CloseMainMenu)
void __cdecl PlayerCmd_CloseMainMenu(scr_entref_t entref)
{
    ...
    {
        iMenuIndex = GScr_GetScriptMenuIndex(Scr_GetString(0, SCRIPTINSTANCE_SERVER));
        iassert(iMenuIndex < 32);
    }
    Com_sprintf(svcmd, sizeof(svcmd), "%c %i", 'Z', iMenuIndex);
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
    Scr_AddInt(1, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A3AF8 ScrCmd_SetAutoPickup)
void __cdecl ScrCmd_SetAutoPickup(scr_entref_t entref)
{
    ...
    enable = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    *(int *)((char *)&pSelf->client->ps + 7372) = enable ? 0 : 1;
}

// Decomp: CoDSP_rdBlackOps.map.c (825A4180 PlayerCmd_GetWeaponRenderOptions)
void __cdecl PlayerCmd_GetWeaponRenderOptions(scr_entref_t entref)
{
    ...
    for ( i = 0; i < 15; ++i )
    {
        if ( pSelf->client->ps.heldWeapons[i].weapon == weaponIndex )
        {
            Scr_AddInt(pSelf->client->ps.heldWeapons[i].options.i, SCRIPTINSTANCE_SERVER);
            return;
        }
    }
}

// Decomp: CoDSP_rdBlackOps.map.c (825A5D08 PlayerCmd_DisableWeaponFire)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A5DF0 PlayerCmd_EnableWeaponReload)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A5ED8 PlayerCmd_DisableWeaponReload)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A5FC0 PlayerCmd_EnableWeaponFire)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A6448 PlayerCmd_NightVisionForceOff)
void __cdecl PlayerCmd_NightVisionForceOff(scr_entref_t entref)
{
    ...
    pSelf->client->ps.pm_flags &= ~0x40u;
}

// Decomp: CoDSP_rdBlackOps.map.c (825A6E10 PlayerCmd_ForceViewmodelAnimation)
void __cdecl PlayerCmd_ForceViewmodelAnimation(scr_entref_t entref)
{
    ...
    weaponIndex = G_GetWeaponIndexForName(weaponName);
    Scr_VerifyWeaponIndex(weaponIndex, weaponName);
    animName = Scr_GetString(1, SCRIPTINSTANCE_SERVER);
    if ( !I_stricmp(animName, "reload") )
        animType = 11;
    else if ( !I_stricmp(animName, "fire") )
        animType = 6;
    else if ( !I_stricmp(animName, "NVG_up") )
        animType = 35;
    else if ( !I_stricmp(animName, "NVG_down") )
        animType = 34;
    else
        Scr_Error(va("Animation name \"%s\" is not supported by this function.\n", animName), SCRIPTINSTANCE_SERVER);
    pSelf->client->ps.weapFlags |= 0x400000u;
    *(unsigned int *)((char *)&pSelf->client->ps + 1264) = weaponIndex;
    *(unsigned int *)((char *)&pSelf->client->ps + 1268) = animType;
}

// Decomp: CoDSP_rdBlackOps.map.c (825A6FF8 PlayerCmd_DisableTurretDismount)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A70E0 PlayerCmd_EnableTurretDismount)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A73B0 PlayerCmd_UploadTime)
void __cdecl PlayerCmd_UploadTime(scr_entref_t entref)
{
    ...
    if ( Scr_GetNumParam(SCRIPTINSTANCE_SERVER) != 2 )
        Scr_Error("Incorrect number of parameters\n", SCRIPTINSTANCE_SERVER);
    missionIndex = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    time = Scr_GetFloat(1, SCRIPTINSTANCE_SERVER);
    if ( time < 0.0f )
        Scr_Error("Negative time is invalid\n", SCRIPTINSTANCE_SERVER);
    Com_sprintf(svcmd, sizeof(svcmd), "%c %i %i", '7', missionIndex, (int)(time * -100.0f));
    SV_GameSendServerCommand(entref.entnum, SV_CMD_RELIABLE, svcmd);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A81D8 PlayerCmd_ClearDamageIndicator)
...
// Decomp: CoDSP_rdBlackOps.map.c (825A95D0 PlayerCmd_UndoLastStand)
void __cdecl PlayerCmd_UndoLastStand(scr_entref_t entref)
{
    ...
    pSelf->client->revive = 0;
    pSelf->client->lastStand = 0;
    pSelf->client->lastStandTime = 0;
    clientState = G_GetClientState(pSelf->s.number);
    clientState->lastStandStartTime = 0;
    clientState->needsRevive = 0;
}

// Decomp: CoDSP_rdBlackOps.map.c (825A96F0 PlayerCmd_GetCurrentArcadeHighScore)
void __cdecl PlayerCmd_GetCurrentArcadeHighScore(scr_entref_t entref)
{
    ...
    iassert(pSelf->client->ps.clientNum < 4);
    xuid = SV_GetPlayerXuid(pSelf->client->sess.cs.clientIndex);
    missionIndex = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    score = 0;
    if ( xuid )
        score = GetPlayerScore(xuid, missionIndex);
    Scr_AddInt(score, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A97B0 PlayerCmd_GetScoreMultiplier)
void __cdecl PlayerCmd_GetScoreMultiplier(scr_entref_t entref)
{
    ...
    iassert(pSelf->client->ps.clientNum < 4);
    clientState = G_GetClientState(pSelf->s.number);
    Scr_AddInt(clientState->clientUIVisibilityFlags, SCRIPTINSTANCE_SERVER);
}

// Decomp: CoDSP_rdBlackOps.map.c (825A9840 PlayerCmd_SetScoreMultiplier)
void __cdecl PlayerCmd_SetScoreMultiplier(scr_entref_t entref)
{
    ...
    iassert(pSelf->client->ps.clientNum < 4);
    multiplier = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    clientState = G_GetClientState(pSelf->s.number);
    clientState->clientUIVisibilityFlags = multiplier;
}

// Decomp: CoDSP_rdBlackOps.map.c (825AA740 PlayerCmd_SetLowReady)
void __cdecl PlayerCmd_SetLowReady(scr_entref_t entref)
{
    ...
    if ( enable )
    {
        ...
        if ( (ps->pm_flags & 0x20000000) != 0 )
        {
            ps->pm_flags &= ~0x20000000u;
...
// Decomp: CoDSP_rdBlackOps.map.c (825AA8E8 PlayerCmd_SetLoweredWeapon)
void __cdecl PlayerCmd_SetLoweredWeapon(scr_entref_t entref)
{
    ...
    enable = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    if ( enable )
        pSelf->client->ps.eFlags |= 0x800000u;
    else
        pSelf->client->ps.eFlags &= ~0x800000u;
}

// Decomp: CoDSP_rdBlackOps.map.c (825AAA08 PlayerCmd_SetScriptHintString)
void __cdecl PlayerCmd_SetScriptHintString(scr_entref_t entref)
{
    ...
    if ( Scr_GetType(0, SCRIPTINSTANCE_SERVER) == 2
        && !I_stricmp(Scr_GetString(0, SCRIPTINSTANCE_SERVER), "") )
    {
        pSelf->client->ps.scriptCursorHintString = 0;
        return;
    }
    Scr_ConstructMessageString(
        0,
        Scr_GetNumParam(SCRIPTINSTANCE_SERVER) - 1,
        "Hint String",
        szHint,
        0x400u);
    if ( !G_GetHintStringIndex(&hintIndex, szHint) )
        Scr_Error(va("Too many different hintstring values. Max allowed is %i different strings", 96), SCRIPTINSTANCE_SERVER);
    pSelf->client->ps.scriptCursorHintString = hintIndex + 1;
}

// Decomp: CoDSP_rdBlackOps.map.c (825AABB8 PlayerCmd_AllowPickupWeapons)
void __cdecl PlayerCmd_AllowPickupWeapons(scr_entref_t entref)
{
    ...
    allow = Scr_GetInt(0, SCRIPTINSTANCE_SERVER);
    if ( allow )
        pSelf->client->ps.eFlags &= ~0x8000000u;
    else
        pSelf->client->ps.eFlags |= 0x8000000u;
}
