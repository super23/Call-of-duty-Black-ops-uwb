...
void __cdecl CG_ParseServerInfo(int localClientNum)
{
    ...
    if ( (cgs->maxclients < 1 || cgs->maxclients > 32)
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\cgame\\\cg_servercmds_sp.cpp",
                    142,
                    0,
                    "%s\n\t(cgs->maxclients) = %i",
...
// Decomp: CoDSP_rdBlackOps.map.c (CG_ParseSunLight @ 823DD8B0)
void __cdecl CG_ParseSunLight()
{
    char *configString;
    float diffuse[3];
    float specular[3];
    ...
    configString = CL_GetConfigString(8);
    if ( !*configString )
    {
    ...
    if ( argCount != 6
        && !Assert_MyHandler(
                "C:\\projects_pc\\cod\\codsrc\\src\\cgame\\cg_servercmds.cpp",
                0x6Fu,
                0,
                "%s\n\t(argCount) = %i",
                "(argCount == 6)",
...
void __cdecl CG_ParseFog(int localClientNum)
{
    ...
    token = Info_ValueForKey((char *)info, va("%i", localClientNum));
    info = token;
                ...
                int height)
{
    ...
    if ( cg_scr_data.airsupport )
    {
        ...
        Scr_AddInt(localClientNum, SCRIPTINSTANCE_CLIENT);
        t = Scr_ExecThread(SCRIPTINSTANCE_CLIENT, cg_scr_data.airsupport, 0xCu);
        Scr_FreeThread(t, SCRIPTINSTANCE_CLIENT);
    }
}
...
void __cdecl CG_DeployServerCommand(int localClientNum)
{
    ...
    switch ( *cmd )
    {
        ...
        case 0x7E:
            v54 = Cmd_Argv(1);
            ControllerIndex = Com_LocalClient_GetControllerIndex(localClientNum);
            Live_GiveGamerPicture(ControllerIndex, v54);
            break;
        case 0x74:
        ...
        case 0x76:
            ...
            break;
        case 0x7C:
            CG_StartFadingBlurServerCommand(localClientNum);
            break;
        case 0x41:
                                        ...
                                        else if ( num < 2580 || num >= 2836 )
                                        {
                                            switch ( num )
                                            {
                                                ...
                                                case 1551:
                                                    ...
                                                    break;
                                                case 3148:
                                                    CG_VisionSetConfigString_LastStand(localClientNum);
                                                    break;
                                            }
                                        }
                                        else
...
{
    if ( !text
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\cgame\\\cg_servercmds_sp.cpp", 1550, 0, "%s", "text") )
    {
        __debugbreak();
    }
...
// Decomp: CoDSP_rdBlackOps.map.c (CG_BlurServerCommand ~823E29B8)
// Map: CoDSP_rd.map VA 0x823E29B8 (cg_servercmds.obj)
void __cdecl CG_BlurServerCommand(int localClientNum)
{
    ...
    blurTimeMs = atoi(Cmd_Argv(1));
    blurEndValue = (float)atof(Cmd_Argv(2));
    timeMode = atoi(Cmd_Argv(3));
    priority = atoi(Cmd_Argv(4));
    if ( !blurTimeMs && blurEndValue <= 0.0f && !timeMode && !priority )
    {
        CG_ClearBlur(localClientNum);
        return;
    }
    iassert(blurTimeMs >= 0);
    iassert(blurEndValue >= 0.0f);
    CG_Blur(localClientNum, blurTimeMs, timeMode, priority, blurEndValue);
}

// Decomp: CoDSP_rdBlackOps.map.c (CG_StartFadingBlurServerCommand ~823E2B00)
void __cdecl CG_StartFadingBlurServerCommand(int localClientNum)
{
    const char *v1;
    const char *v2;
    int durationMS;
    ...
    durationMS = atoi(v1);
    v2 = Cmd_Argv(2);
    blurValue = (float)atof(v2);
    if ( durationMS < 0 )
    {
        if ( !Assert_MyHandler(
                    "C:\\projects\\cod\\codsrc\\src\\cgame\\cg_servercmds.cpp",
                    0x799u,
                    0,
                    "%s",
                    "time >= 0") )
    ...
    CG_StartFadingBlur(localClientNum, durationMS, blurValue);
}
