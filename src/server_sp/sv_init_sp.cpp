...
#include <client_sp/cl_mapload_cin_sp.h>
...
void __cdecl SV_SetConfigstring(int index, char *val)
{
    ...
    {
        if ( !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_init_sp.cpp", 156, 0, "%s", "!val") )
            __debugbreak();
    }
}
...
void __cdecl SV_GetConfigstring(unsigned int index, char *buffer, int bufferSize)
{
    ...
    if ( !sv.configstrings[index]
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_init_sp.cpp",
                    247,
                    0,
                    "%s",
...
void __cdecl SV_SetExpectedHunkUsage(char *mapname)
{
    ...
    if ( len >= 0 )
    {
        ...
        while ( 1 )
        {
            ...
            if ( !token
                && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_init_sp.cpp", 724, 0, "%s", "token") )
            {
                __debugbreak();
            }
            if ( !*token )
...
void __cdecl    SV_SpawnServer(int controllerIndex, char *server, int mapIsPreloaded, int savegame)
{
    ...
    if ( !mapIsPreloaded )
    {
        ...
        R_EndRemoteScreenUpdate(0);
        CL_ShutdownAll();
        // CoDSP_rdBlackOps.map.c:6721520-6721524 — play map-load bink (cin_levels.txt / default.bik fallback).
        CL_MapLoadingKickLoadMovie(server, (char)savegame);
    }
    ...
    if ( !mapIsPreloaded )
    {
        ...
        if ( useFastFile->current.enabled )
        {
            Com_SP_UnloadLevelZonesForMapLoad();
            Com_LoadLevelFastFiles(server);
            SV_SetGametype();
            I_strncpyz(sv.gametype, sv_gametype->current.string, 64);
            if ( Com_IsMenuLevel(server) )
                DB_SyncXAssets();
...
#ifdef KISAK_SP
    // Decomp: BlackOps.singleplayer.c — Com_LoadFrontEnd() loads frontend.ff; map frontend must not ERR_DROP without col_map_sp.
    if ( useFastFile->current.enabled && Com_IsMenuLevel(server) )
        Com_SP_LoadMenuLevelServerWorld(server, &checksum);
    else
#endif
    {
...
void __cdecl SV_Init()
{
    ...
    sv_gameskill = _Dvar_RegisterInt("g_gameskill", 1, 0, 3, 0x1064u, "Game skill level");
    // Decomp: BlackOps.singleplayer.c SV_Init @ 0x00698260
    ...
    _Dvar_RegisterInt("player_healthEasy", 500, 10, 2000, 0x2000u, "Player health on easy mode");
    _Dvar_RegisterInt("player_healthMedium", 275, 10, 2000, 0x2000u, "Player health in medium mode");
    _Dvar_RegisterInt("player_healthHard", 165, 10, 2000, 0x2000u, "Player health in challenging mode");
    _Dvar_RegisterInt("player_healthFu", 115, 10, 2000, 0x2000u, "Player health in veteran mode");
    _Dvar_RegisterInt(
...
void __cdecl SV_DropAllClients()
{
    ...
    while ( i < com_maxclients->current.integer )
    {
        if ( drop->header.state >= CS_CONNECTED )
            SV_DropClient(drop, "EXE_DISCONNECTED", 1, 1);
        ++i;
        ++drop;
    }
}
