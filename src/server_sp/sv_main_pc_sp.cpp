...
int __cdecl SV_GetTime()
{
    if ( !s_LSGTime
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_main_pc_sp.cpp",
                    199,
                    0,
                    "%s",
                    "0 != s_LSGTime") )
...
void __cdecl SVC_RemoteCommand(netadr_t from)
{
    ...
    if ( !lasttime || time - lasttime >= 500 )
    {
        ...
        if ( rcon_password->current.string[0] && !strcmp(password, rcon_password->current.string) )
        {
            valid = 1;
            v6 = SV_Cmd_Argv(2);
...
unsigned __int64 __cdecl SV_GetOwnerID()
{
    ...
    retval = 0;
    if ( sv_ownerid && sv_ownerid->current.string[0] )
