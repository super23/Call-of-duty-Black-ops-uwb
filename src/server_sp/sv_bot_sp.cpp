...
void __cdecl SV_BotSwitchWeapon(const client_t *bot, int weapIndex)
{
    if ( !bot->bIsTestClient
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_bot_sp.cpp",
                    2776,
                    0,
                    "%s",
                    "bot->bIsTestClient") )
...
void __cdecl Bot_UpdatePath(bot_info_t *botInfo, const client_t *bot)
{
    ...
    if ( !Path_Exists(&botInfo->path) )
    {
        ...
        {
            node = &gameWorldCurrent->path.nodes[irand(0, gameWorldCurrent->path.nodeCount)];
                    ...
                    if ( Path_Exists(&botInfo->path) || !botInfo->threat.enemy )
                    {
                        ...
                        else
                        {
                            ...
                            if ( Bot_IsAtNegotiationNode(botInfo, bot, &botInfo->path) )
                                vectoangles(
                                    gameWorldCurrent->path.nodes[botInfo->path.pts[botInfo->path.lookaheadNextNode].iNodeNum].constant.forward,
                                    viewAngles);
