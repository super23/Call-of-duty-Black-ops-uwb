    ...
    for ( i = client->reliableSent + 1; ; ++i )
    {
        ...
        if ( client->reliableCommandInfo[index].type )
        {
            if ( client->reliableCommandInfo[index].type != 1
                && !Assert_MyHandler(
                            "C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_main_sp.cpp",
                            350,
                            0,
                            "%s",
...
void __cdecl SV_FreeClientScriptId(client_t *cl)
{
    ...
    if ( !cl->scriptId
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_main_sp.cpp", 2066, 0, "%s", "cl->scriptId") )
    {
        __debugbreak();
    }
...
#ifdef KISAK_SP
    // Decomp: CoDSP_rdBlackOps.map.c SV_PreFrame @ 828D0908 — flush deferred Scr_PlayerConnect/ClientBegin.
    G_FlushPendingClientConnects();
#endif
}
...
void __cdecl SV_FrameInternal(int controllerIndex, int msec)
{
    ...
    if ( msec < 0
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_main_sp.cpp", 2929, 0, "%s", "msec >= 0") )
    {
        __debugbreak();
    }
