...
void __cdecl SV_WriteSnapshotToClient(client_t *client, msg_t *msg)
{
    ...
    if ( !svsHeaderValid
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_snapshot_sp.cpp",
                    849,
                    0,
                    "%s",
                    "svsHeaderValid") )
    ...
    frame = remoteFrame;
    if ( !remoteFrame
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_snapshot_sp.cpp", 874, 0, "%s", "frame") )
    {
        __debugbreak();
    }
                ...
                int *otherFlags)
{
    ...
    if (!health
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_snapshot_sp.cpp", 2610, 0, "%s", "health"))
    {
        __debugbreak();
    }
    if (!otherFlags
    ...
    else
    {
        ...
        if (!ps
            && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\server_sp\\sv_snapshot_sp.cpp", 2627, 0, "%s", "ps"))
        {
            __debugbreak();
        }
