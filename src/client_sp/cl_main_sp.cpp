...
#include "cl_parse_sp.h"
#include <client/cl_devgui.h>
#include <devgui/devgui.h>
#include "cl_cgame_sp.h"
#include "cl_ui_sp.h"
...
#include "cl_main_pc_sp.h"
#include <DynEntity/DynEntity_client.h>
#include <client/cl_cin.h>
#include <gfx_d3d/r_cinematic.h>
#include "cl_input_sp.h"
#include <DW/dwNet.h>
...
void __cdecl CL_WriteUncompressedDemoInfo(int localClientNum)
{
    ...
    if ( !nextConstConfigStringNumber )
    {
        ...
        if ( !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 1603, 0, v4) )
            __debugbreak();
    }
            ...
            if ( i )
            {
                if ( nextConstConfigStringNumber )
                {
                    if ( nextConstConfigStringNumber < i )
                    {
                        ...
                        if ( !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 1688, 0, v5) )
                            __debugbreak();
                    }
                }
                if ( nextConstConfigStringNumber == i )
    ...
    if ( numConfigStringsWritten != configStringCount
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp",
                    1762,
                    0,
                    "%s",
...
void __cdecl CL_MapLoading(const char *mapname)
{
    ...
    if (CL_AnyLocalClientsRunning())
    {
        ...
        {
            for (localClientNuma = 0; localClientNuma < MAX_LOCAL_CLIENTS; ++localClientNuma)
            {
        ...
        else
        {
            ...
            for (localClientNumb = 0; localClientNumb < MAX_LOCAL_CLIENTS; ++localClientNumb)
            {
...
void __cdecl CL_ResetSkeletonCache(unsigned int localClientNum)
{
    ...
    if ( !clients
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 2640, 0, "%s", "clients") )
    {
        __debugbreak();
    }
...
void __cdecl LoadMapLoadscreen(char *mapname)
{
    ...
    {
        Com_PrintWarning(14, "LoadMapLoadscreen called with empty map name; skipping map loadscreen zone prep\n");
        return;
    }

    DB_AddUserMapDir(mapname);
    // CoDSP_rdBlackOps.map.c:6757777-6757781 / 6749605-6749609 — loadscreen material + sync before UI map set.
    Com_LoadMapLoadingScreenFastFile(mapname);
    DB_SyncXAssets();
    DB_UpdateDebugZone();
    UI_SetMap(mapname, (char *)"");
...
void __cdecl CL_NextDownload(int localClientNum)
{
    ...
    if (!cls.downloadList[0])
        goto LABEL_11;
    //if (com_sv_running->current.enabled)
    //    MyAssertHandler(".\\client_sp\\cl_main_sp.cpp", 2721, 0, "%s", "!com_sv_running->current.enabled");
...
void __cdecl CL_BuildMd5StrFromCDKey(char *md5Str)
{
    ...
    if ( !md5Str
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 3157, 0, "%s", "md5Str") )
    {
        __debugbreak();
    }
...
void __cdecl CL_DisconnectError(const char *message)
{
    ...
    if ( !message
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 4651, 0, "%s", "message") )
    {
        __debugbreak();
    }
            ...
            if ( connstate < CA_CONNECTED )
            {
                if ( connstate == CA_CHALLENGING )
                {
                    ...
                    if ( NET_CompareBaseAdr(from, clc->serverAddress) )
                    {
                        ...
                        {
                            CL_SetLocalClientConnectionState(localClientNum, CA_CONNECTED);
                        }
#endif
                        else
                        {
...
void __cdecl CL_WriteDemoMessage(int localClientNum, msg_t *msg, int headerBytes)
{
    ...
    len = swlen;
    if ( swlen < 0
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 905, 0, "%s", "len >= 0") )
    {
        __debugbreak();
    }
    FS_WriteToDemo((char *)&swlen, 4u, clc->demofile);
    ...
    if ( CG_IsShowingZombieMap() || Com_LocalClient_IsBeingUsed(localClientNum) )
    {
        if ( _InterlockedExchange(&cls.scriptError, 0) )
        {
            ...
            if ( strlen(buf) )
            {
                ...
                {
#ifdef KISAK_SP
                    if ( connstate == CA_ACTIVE && zombiemode->current.enabled )
                    {
                        Com_Printf(0, "ZM script error (suppressed popup): %s\n", buf);
                        Com_SetErrorMessage(";");
                    }
                    else
#endif
...
{
    if ( !config
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 6465, 0, "%s", "config") )
    {
        __debugbreak();
    }
...
void __cdecl CL_startSingleplayer_f()
{
    ...
    // KisakBlackSP is the unified SP client; retail MP handoff via CoDSP_rd.exe is not used here.
}
...
void __cdecl CL_StopLogoOrCinematic(int localClientNum)
{
    ...
    else if ( clcState == CA_LOGO )
    {
        CL_StopLogo(localClientNum);
    }
#ifdef KISAK_SP
    else if ( clcState == CA_DISCONNECTED
        && com_startupIntroPlayed
        && !com_startupIntroPlayed->current.enabled )
    {
        // Startup intro skipped or finished without entering CA_LOGO/CA_CINEMATIC.
    }
#endif
    else
    {
        if ( !Assert_MyHandler(
                        ...
                        0,
                        "%s",
                        "clcState == CA_LOGO || clcState == CA_CINEMATIC") )
        {
            __debugbreak();
        }
    }
    ...
    if ( clcState == CA_LOGO
#ifdef KISAK_SP
        || (clcState == CA_CINEMATIC && com_introPlayed && !com_introPlayed->current.enabled)
        || (clcState == CA_DISCONNECTED
            && com_startupIntroPlayed
            && !com_startupIntroPlayed->current.enabled)
#endif
    )
...
char __cdecl Playlist_ReadFromDisk()
{
    ...
    if ( FS_ReadFile("playlists_sp.info", &playlistBuffer) )
    {
        if ( playlistBuffer )
        {
...
void __cdecl CL_PlayCinematic_f()
{
    ...
    if ( Cmd_Argc() < 2 )
    {
        Com_Printf(0, "USAGE: cinematic <name> [playbackFlags]\n");
        return;
    }
    name = Cmd_Argv(1);
    playbackFlags = 0;
    if ( Cmd_Argc() > 2 )
        playbackFlags = (unsigned int)atoi(Cmd_Argv(2));
    connstate = CL_GetLocalClientConnectionState(0);
    if ( connstate == CA_CINEMATIC )
        ...
        return;
    CL_SetLocalClientConnectionState(0, CA_CINEMATIC);
    if ( cls.uiStarted )
        UI_SetActiveMenu(0, UIMENU_NONE);
    SND_StopSounds(SND_STOP_ALL);
...
bool __cdecl CL_GetServerList(int source, serverInfo_t **servers, int **count)
{
    ...
    if ( !servers
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 7967, 0, "%s", "servers") )
    {
        __debugbreak();
    }
    if ( !count
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_sp.cpp", 7968, 0, "%s", "count") )
    {
        __debugbreak();
    }
    retval = 0;
