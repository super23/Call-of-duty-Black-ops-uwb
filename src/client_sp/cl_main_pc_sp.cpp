...
#ifndef KISAK_DW
static serverInfo_t *__cdecl FindServerBySecID(bdSecurityID *)
{
    return 0;
}

static bool __cdecl dwIsPagedFindInProgress()
{
    return false;
}

static void __cdecl dwXnaddrtonetadr(unsigned __int8 *, netadr_t *privadr, netadr_t *pubadr)
{
    if ( privadr )
        memset(privadr, 0, sizeof(*privadr));
    if ( pubadr )
        memset(pubadr, 0, sizeof(*pubadr));
}

static int __cdecl dwRawSendTo(netadr_t *, unsigned __int8 *, unsigned int)
{
    return 0;
}

static bool __cdecl Live_FindFavouritesInProgress()
{
    return false;
}

static void __cdecl Live_FindFavouriteServers()
{
}

static void __cdecl Live_FindFriendServers()
{
}

static void __cdecl Live_FindRecentServers()
{
}

static void __cdecl Live_FindSessionsStart(bool, int)
{
}
#endif
...
void __cdecl CL_ServerInfoPacket(bdSecurityID *secID, msg_t *msg, int time)
{
    ...
    if ( !CL_GetServerList(cls.pingUpdateSource, &servers, &numServers) || !servers )
        return;

    infoString = MSG_ReadString(msg, strBuf, sizeof(strBuf));
    protocolValue = Info_ValueForKey(infoString, "protocol");
    protocol = atoi(protocolValue);
    protocolDvar = Dvar_GetString("debug_protocol");
    expectedProtocol = *protocolDvar ? atoi(protocolDvar) : 1044;
    ...
    if ( !server )
    {
        Com_DPrintf(14, "Got ping response from unknown server secid\n");
        return;
    }
...
{
    netadr_t privServerAddr;
    netadr_t pubServerAddr;
    unsigned __int8 sendBuf[9];

    memset(sendBuf, 0, sizeof(sendBuf));
    if ( !CL_RawPingSetupBuffer(sendBuf, sizeof(sendBuf), opcode, &server->xnkid) )
        return;

    server->pingedTime = Sys_Milliseconds();
    dwXnaddrtonetadr(server->xnaddr.addrBuff, &privServerAddr, &pubServerAddr);
    dwRawSendTo(&pubServerAddr, sendBuf, sizeof(sendBuf));
    if ( !NET_CompareAdr(privServerAddr, pubServerAddr) )
        dwRawSendTo(&privServerAddr, sendBuf, sizeof(sendBuf));
}
...
void __cdecl CL_FindServers_f()
{
    ...
    if ( !CL_GetServerList(serverType, &servers, &count) || !servers || !count )
        return;
...
void CL_RconHost()
{
    ...
    if ( Cmd_Argc() == 3 )
    {
        ...
        if ( NET_StringToAdr(hostName, &rconGlob.host) )
        {
            if ( rconGlob.host.type == NA_BAD
                && !Assert_MyHandler(
                            "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_pc_sp.cpp",
                            1185,
                            1,
                            "%s",
                            "rconGlob.host.type != NA_BAD") )
...
{
    bdSecurityID secId;
    serverStatusInfoResponse_s *serverStatus;
    serverInfo_t *server;
    int i;
    ...
    if ( !serversecurityID )
    {
        for ( i = 0; i < 16; ++i )
        {
            ...
            memset(&cl_serverStatusList[i].secId, 0, sizeof(cl_serverStatusList[i].secId));
        }
        return 0;
    }

    memcpy(&secId, serversecurityID, sizeof(secId));
    server = FindServerBySecID(&secId);
    if ( !server )
        return 0;
    ...
    if ( secId == serverStatus->secId )
    {
        ...
        if ( serverStatus->startTime < (int)(Sys_Milliseconds() - cl_serverStatusResendTime->current.integer) )
        {
...
int __cdecl CL_ServerStatusScoreBoard(char *serversecurityID, char *serverStatusString, int maxLen)
{
    ...
    if ( !serversecurityID )
    {
        for ( i = 0; i < 16; ++i )
        {
            ...
            memset(&cl_serverStatusScoreBoardList[i].secId, 0, sizeof(cl_serverStatusScoreBoardList[i].secId));
        }
        return 0;
    }
...
void __cdecl CL_QuickMatch_PingServers()
{
    ...
    if ( !cl_maxppf
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_main_pc_sp.cpp", 2809, 0, "%s", "cl_maxppf") )
    {
        __debugbreak();
    }
    i = 0;
...
void __cdecl CL_QuickMatch_Frame()
{
    ...
    switch ( s_quickmatchstate )
    {
        ...
        case QM_FAILED:
            ...
            UI_OpenToastPopup(0, "menu_sp_killstreak_select", v0, v1, 2700);
            s_quickmatchstate = QM_DORMANT;
            break;
        default:
