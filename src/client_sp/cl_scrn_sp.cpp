...
LABEL_31:
                    ...
                    case CA_DISCONNECTED:
                        ...
                        if ( Sys_IsMainThread() )
                        {
                            if ( !CL_AllLocalClientsDisconnected()
                                && !Assert_MyHandler(
                                            "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_scrn_sp.cpp",
                                            718,
                                            0,
                                            "%s",
    ...
    // CoDSP @82447390 — SCR_DrawPleaseWait when long_blocking_call / network_warning dvars set.
    if ( long_blocking_call && long_blocking_call->current.enabled )
        SCR_DrawPleaseWait();
    else if ( cl_network_warning && cl_network_warning->current.enabled )
        SCR_DrawPleaseWait();
    else if ( sv_network_warning && sv_network_warning->current.enabled )
        SCR_DrawPleaseWait();
}
...
// CoDSP @82446150 — spinner + PLATFORM_PLEASEWAIT during long_blocking_call (frontend spinner path).
void SCR_DrawPleaseWait()
{
    ...
    pleaseWaitText = UI_SafeTranslateString("PLATFORM_PLEASEWAIT");
    font = UI_GetFontHandle(&scrPlaceFull, 0, 0.5f);
    scale = 0.69999999f;
    textWidth = (float)UI_TextWidth(pleaseWaitText, 0, font, scale);
    textHeight = (float)R_TextHeight(font) * scale;
    CL_GetScreenDimensions(&screenWidth, &screenHeight, &aspect);
    spinnerSize = textHeight;
    x = ((float)screenWidth - textWidth) * 0.5f;
    y = (float)screenHeight * 0.9f - textHeight;
    color[0] = 1.0f;
    color[1] = 1.0f;
    color[2] = 1.0f;
    color[3] = 1.0f;
    CL_DrawSpinnerPhysical(
        ...
        spinnerSize,
        color);
    R_AddCmdDrawText(pleaseWaitText, 0x7FFFFFFF, font, x, y, scale, scale, 0.0f, color, 3);
}
...
void    HiResScreenshot(int tiles)
{
    ...
    if ( !tiles
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_scrn_sp.cpp", 1769, 0, "%s", "tiles != 0") )
    {
        __debugbreak();
    }
    if ( tiles )
