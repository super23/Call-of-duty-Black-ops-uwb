...
bool __cdecl CL_KeyActiveOrPressed(int localClientNum, unsigned int kbButton)
{
    if ( kbButton >= 0x2F
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_input_sp.cpp",
                    2284,
                    0,
...
void __cdecl CL_AdjustAngles(int localClientNum)
{
    ...
    kb = playersKb[localClientNum];
    LocalClientGlobals = CL_GetLocalClientGlobals(localClientNum);
    // BlackOpsMP.retail.c:~813128 / ~813233: freezecontrols (0x800) and freezecontrolsallowlook (0x10) block look.
...
double __cdecl CL_KeyState(kbutton_t *key)
{
    ...
    if ( msec >= frame_msec )
        return 1.0;
    if ( !frame_msec
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_input_sp.cpp", 313, 0, "%s", "frame_msec") )
    {
        __debugbreak();
    }
...
void __cdecl CL_GetMouseMovement(clientActive_t *cl, float *mx, float *my)
{
    ...
    if ( !mx && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_input_sp.cpp", 2148, 0, "%s", "mx") )
        __debugbreak();
    if ( !my && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_input_sp.cpp", 2149, 0, "%s", "my") )
        __debugbreak();
...
void __cdecl IN_Attack_Down()
{
    ...
    if ( !client
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_input_sp.cpp", 546, 0, "%s", "client") )
    {
        __debugbreak();
    }
