...
void __cdecl CL_InitUI()
{
    ...
    // Decomp: CoDSP_rdBlackOps.map.c CL_InitUI — SP loads ui_sp/ menu zones (see UI_InitUIInfos + KISAK_SP).
    // Offline profile init runs after UI_RegisterDvars (UI_InitOnceForAllClients), not here: Live_Init is
    // omitted on SP and MP-only profile dvars are not registered yet (see GamerProfile_InitSPOfflineProfile).
    UI_InitOnceForAllClients();
    for ( localClientNum = 0; localClientNum < MAX_LOCAL_CLIENTS; ++localClientNum )
        UI_Init(localClientNum);
    Dvar_SetBoolByName("ui_multiplayer", 0);
    cls.uiStarted = 1;
