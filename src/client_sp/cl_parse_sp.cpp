                ...
                int movementDir)
{
    ...
    if ( v7 >= 0x100
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_parse_sp.cpp",
                    106,
                    0,
...
void __cdecl CL_ParseGamestate(int localClientNum, msg_t *msg)
{
    ...
    if (clc->clientNum >= com_maxclients->current.integer)
    {
...
{
    const int parseEntityIndex = cl->parseEntitiesNum & 0x7FF;
    memcpy(&cl->parseEntities[parseEntityIndex], old, sizeof(cl->parseEntities[parseEntityIndex]));
    ++cl->parseEntitiesNum;
    ++frame->numEntities;
}
