...
void __cdecl LAN_GetServerAddressString(int source, int n, char *buf, int buflen)
{
    ...
    if ( !buf && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_ui_pc_sp.cpp", 108, 0, "%s", "buf") )
        __debugbreak();
    servers = 0;
...
void __cdecl LAN_GetServerSecurityId(int source, int n, bdSecurityID *buf, int buflen)
{
    ...
    if ( !buf && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_ui_pc_sp.cpp", 126, 0, "%s", "buf") )
        __debugbreak();
    if ( buflen < 8
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_ui_pc_sp.cpp",
                    127,
                    0,
                    "%s",
    ...
    if ( server && buf )
    {
        *buf = 0;
        ...
        if ( source == 4 )
        {
            ...
            {
                unsigned __int64 userId = userIDs[i];
                Friends_GetByID(0, userId, &finfo);
                int written = sprintf_s(strptr, 128 - stringpos, "%s\n", finfo.name);
                stringpos += written;
            ...
            {
                int ellipsisWritten = sprintf_s(strptr, 128 - stringpos, "...");
                stringpos += ellipsisWritten;
                strptr = &userString[stringpos];
            }
