...
char *__cdecl CL_AllocSkelMemory(unsigned int size)
{
    ...
    if ( !size && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_pose_sp.cpp", 33, 0, "%s", "size") )
        __debugbreak();
    sizea = (size + 15) & 0xFFFFFFF0;
    if ( sizea > 0x3FFF0
        && !Assert_MyHandler(
                    "C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_pose_sp.cpp",
                    36,
                    0,
                    "%s",
    ...
    if ( sizea + skelMemPos > 0x3FFF0 )
        return 0;
    if ( !result
        && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_pose_sp.cpp", 63, 0, "%s", "result") )
    {
        __debugbreak();
    }
    return result;
...
int __cdecl CL_DObjCreateSkelForBones(const DObj *obj, int *partBits, DObjAnimMat **pMatOut)
{
    ...
    if ( !obj && !Assert_MyHandler("C:\\projects_pc\\cod\\codsrc\\src\\client_sp\\cl_pose_sp.cpp", 95, 0, "%s", "obj") )
        __debugbreak();
